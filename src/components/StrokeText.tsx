...
export const StrokeText = ({
  ...
  const fontSizeNum = typeof fontSize === 'number' ? fontSize : parseFloat(fontSize) || 128;
  const dash = Math.max(fontSizeNum * 7, 200);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, fontSizeNum * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  }, [characters, fontSizeNum, fontWeight, letterSpacing, strokeWidth]);
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontSizeNum} 600 ${fontSizeNum * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${Math.round(fontSizeNum * 1.3)}px` } as CSSProperties}
      role="img"
